...
현재 저장소의 Docker Compose 구성은 로컬 개발용 PostgreSQL 실행을 담당한다. 애플리케이션은 로컬에서는 Gradle로 실행하고, 운영 배포 시 애플리케이션 컨테이너 구성을 추가할 수 있다.
...
| `POSTGRES_PORT` | 로컬 PostgreSQL 공개 포트 |
| `POSTGRES_DB` | Docker Compose PostgreSQL DB 이름 |
| `POSTGRES_USER` | Docker Compose PostgreSQL 사용자 |
