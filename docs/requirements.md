...
실시간 앱, 별도 모바일 앱, 복잡한 대기열 인프라는 사용하지 않는다. 사용자는 웹 페이지의 새로고침 버튼으로 남은 순서를 확인하고, 관리자가 호출하거나 노쇼 처리하면 카카오 알림톡을 받는다.
...
카카오 알림톡은 관리자가 관리자 화면에서 대기자를 직접 호출하거나 노쇼 처리할 때 발송한다. 호출 알림과 노쇼 알림은 각각 승인된 템플릿 ID를 사용하며, 호출 알림은 중복 발송 방지를 위해 발송 완료 시각을 DB에 저장한다.
...
9. 관리자가 노쇼 처리하면 해당 대기자에게 노쇼 카카오 알림톡을 발송한다.
10. 입장완료 또는 노쇼 처리된 사용자는 별도 처리 완료 화면에서 모아서 확인한다.
...
- 호출 카카오 알림톡은 관리자가 관리자 화면에서 개별 호출 버튼 또는 부족 인원 호출 버튼을 눌렀을 때 발송한다.
- 호출 카카오 알림톡 발송에는 승인된 호출 템플릿 ID를 사용한다.
- 호출 카카오 알림톡은 `WAITING` 상태이면서 `notified_at`이 없는 사용자에게만 발송한다.
- 호출 카카오 알림톡은 한 사용자에게 한 번만 발송한다.
- 호출 알림톡 중복 발송 방지를 위해 `notified_at`을 저장한다.
- 호출 카카오 알림톡 발송 성공 후 상태를 `CALLED`로 변경한다.
- 호출 카카오 알림톡 발송 실패 시 로그를 남기고, `notified_at`은 저장하지 않는다.
- 노쇼 카카오 알림톡은 관리자가 `CALLED` 또는 `ARRIVED` 상태의 대기자를 노쇼 처리할 때 발송한다.
- 노쇼 카카오 알림톡 발송에는 승인된 노쇼 템플릿 ID를 사용한다.
- 노쇼 카카오 알림톡 발송 실패 시 로그를 남기고, 노쇼 상태 변경은 확정하지 않는다.
...
- 노쇼 처리 시 해당 대기자에게 별도 노쇼 카카오 알림톡을 발송한다.
...
| `notified_at` | timestamp | 호출 카카오 알림톡 발송 완료 시각 |
...
services:
  app:
    ...
    environment:
      ...
      SOLAPI_KAKAO_CALL_TEMPLATE_ID: ${SOLAPI_KAKAO_CALL_TEMPLATE_ID}
      SOLAPI_KAKAO_NO_SHOW_TEMPLATE_ID: ${SOLAPI_KAKAO_NO_SHOW_TEMPLATE_ID}
...
| `SOLAPI_KAKAO_CALL_TEMPLATE_ID` | 승인된 호출 카카오 알림톡 템플릿 ID |
| `SOLAPI_KAKAO_NO_SHOW_TEMPLATE_ID` | 승인된 노쇼 카카오 알림톡 템플릿 ID |
